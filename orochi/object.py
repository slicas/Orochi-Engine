...
class CollisionBody:
    def __init__(self,object,ID : str,body_type,x_shift = 0,y_shift = 0):
        self.object = object
        ...
        self.__type = type(body_type).__name__
        match self.__type:
            case 'QuadBody': 
                self.width = body_type.width
                self.height = body_type.height
            case "EllipseBody":
                    self.radius_x = body_type.radius_x
                    self.radius_y = body_type.radius_y
        self.body = pr.Rectangle(self.x,self.y,self.width,self.width) if self.__type == "QuadBody" else orochi.math.Ellipse(self.x,self.y,self.radius_x,self.radius_y)
        self.ID = ID
    def get_type(self):
        return self.__type
    def show_collision_shape(self):
        match self.__type:
            case "QuadBody" : pr.draw_rectangle_lines(self.x,self.y,self.width,self.height,(0,255,0,255))
            case "EllipseBody" : pr.draw_ellipse_lines(self.x,self.y,self.radius_x,self.radius_y,(0,255,0,255))
    def update(self):
        match self.__type:
            case "QuadBody":
...
class Object:
    def __init__(self,game,x,y,width,height,image = None,name = "unamed",begin_with_collision_body = False):
        ...
        self.angle = 0
        self.origin = 0.0
        self.__collisions_bodies = [CollisionBody(self,"Body",QuadBody(self.width,self.height))] if begin_with_collision_body else []
        ...
        self.render_shift_width = 0
        self.render_shift_height = 0
        self.render_shift_width = 0
        self.render_shift_height = 0
        if(image != None):
    ...
    def x_scale(self,value):
        self.width /= abs(self._x_scale)
        self.width *= abs(value)
        self._x_scale = value
    @y_scale.setter
    def y_scale(self,value):
        self.height /= abs(self._y_scale)
    ...
    def render(self):
        ...
        if(self.image != None):
            ...
            pr.draw_texture_pro(self.image,pr.Rectangle(self.frame_x,self.frame_y,self.frame_width*flip_x,self.frame_height*flip_y),pr.Rectangle(self.x + self.render_shift_x,self.y + self.render_shift_y,self.width + self.render_shift_width,self.height + self.render_shift_height),pr.Vector2((self.width+self.render_shift_width)*self.origin, (self.height+self.render_shift_height)*self.origin),self.angle,self.tint)
    def get_id(self):
        return self.__ID
