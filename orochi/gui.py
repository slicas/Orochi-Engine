...
class Image(GuiComponent):
    def __init__(self,game,theme,x,y,texture,width,height,origin = (.5,.5),angle = 0,tint = (255,255,255,255)):
        super().__init__(game,theme,x,y)
    ...
    def mouse_enter(self):
        if(self._visible):
            return rect_collision(pr.Rectangle(self.x - (self.width*self.origin[0]),self.y- (self.height*self.origin[1]),self.width,self.height),pr.Rectangle(get_gui_mouse_x(),get_gui_mouse_y(),1,1))
    def is_pressed(self):
            if(self._visible):
                if(self.mouse_enter() and is_mouse_button_pressed(MB_LEFT) and not self.disabled):
                    return True
    ...
    def is_down(self):
        if(self._visible):
            if(self.mouse_enter() and is_mouse_button_down(MB_LEFT) and not self.disabled):
                ...
                return False
    def render(self):
        pr.draw_texture_pro(self.texture,pr.Rectangle(0,0,self.width,self.height),pr.Rectangle(self.x,self.y,self.width,self.height),(self.width*self.origin[0],self.height*self.origin[1]),self.angle,self.tint)
    def update(self):
        pass
...
class Button(GuiComponent):
    ...
    def render(self):
        ...
        draw_text(self.theme.font,self.text,self.x,self.y,(.5,.5),0,self.font_size,1,self.font_color)
